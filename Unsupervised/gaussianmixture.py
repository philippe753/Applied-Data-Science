...
imbalanced_sample=pd.DataFrame(full_credit_card_data).sample(n=10000,random_state=5)
def counter(array):
      count=0
      for i in array:
            if i==1:
...
def gaussianmix_function(dataset,n_components=2):
      gaussianmix_dataframe=pd.DataFrame([])
      gaussianmix_dataframe['True labels']=dataset['Class']
      true_labels=np.array(gaussianmix_dataframe['True labels'])
      ...
      gaussianmix_dataframe['Component 1']=X_pca_dataframe[0]
      gaussianmix_dataframe['Component 2']=X_pca_dataframe[1]
      gaussianmix_dataframe['Component 3']=X_pca_dataframe[2]
      gaussianmix_dataframe['Component 4']=X_pca_dataframe[3]
      ...
      gaussianmix_dataframe['results']=birch_prediction
      ...
      gaussianmix_dataframe['Class']=gaussianmix_dataframe['results'].replace(0,'Non-Fraud')
      gaussianmix_dataframe['Class']=gaussianmix_dataframe['Class'].replace(1,'Fraud')
      print('Dataframe',gaussianmix_dataframe)
      plt.figure(7)

      sns.scatterplot(gaussianmix_dataframe['Component 1'],gaussianmix_dataframe['Component 2'],
            hue='Class',
            data=gaussianmix_dataframe)
      ...
      print('Accuracy score',metrics.accuracy_score(np.array(gaussianmix_dataframe['True labels']),np.array(gaussianmix_dataframe['results'])))
      print('Confusion matrix',metrics.confusion_matrix(np.array(gaussianmix_dataframe['True labels']),np.array(gaussianmix_dataframe['results'])))
      plt.show()

gaussianmix_function(dataset)
